...
IntStrength = 0.8; % scaling factor for interaction strength
...
for ne = 1:Ne
    ...
    ct = 0;
    for nr = 1:Nr
        odefun = @(t, S) (ones(N,1) - S./K + ai*S).*r.*S;
    ...
    aisi = [aisi; [-(rand(1,Ns)<IntConnectivity).*(IntStrength-0.5*sprd+sprd*rand(1,Ns)), 0]];
    ...
    cti = 0;
    for nri = 1:Nri
        odefun = @(t, S) (ones(Ni,1) - S./Ksi + aisi*S).*rsi.*S;
...
set(gca,'FontSize',12)
saveas(gcf,strcat('InvasionSuccess_ai_Nsp',num2str(Nsp),'_Str',num2str(round(10*IntStrength)),'_Conn',num2str(100*IntConnectivity),'_Sprd',num2str(10*sprd)),'fig')
...
save(strcat('CheckInvasion_LV_ai_std_Nsp',num2str(Nsp),'_Nr',num2str(Nr),'_Nri',num2str(Nri),'_dil',num2str(d),'_IntStrength',num2str(IntStrength),'_IntConnectivity',num2str(IntConnectivity),'_Sprd',num2str(sprd),'.mat'))
